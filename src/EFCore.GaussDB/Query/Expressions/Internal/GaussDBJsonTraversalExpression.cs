...
/// </summary>
public class GaussDBJsonTraversalExpression : SqlExpression, IEquatable<GaussDBJsonTraversalExpression>
{
    ...
    /// <summary>
    ///     Constructs a <see cref="GaussDBJsonTraversalExpression" />.
    /// </summary>
    public GaussDBJsonTraversalExpression(
    ...
    /// </summary>
    public virtual GaussDBJsonTraversalExpression Update(SqlExpression expression, IReadOnlyList<SqlExpression> path)
        => expression == Expression && path.Count == Path.Count && path.Zip(Path, (x, y) => (x, y)).All(tup => tup.x == tup.y)
            ? this
            : new GaussDBJsonTraversalExpression(expression, path, ReturnsText, Type, TypeMapping);

    /// <inheritdoc />
    public override Expression Quote()
        => New(
            _quotingConstructor ??= typeof(GaussDBJsonTraversalExpression).GetConstructor(
    ...
    /// <summary>
    ///     Appends an additional path component to this <see cref="GaussDBJsonTraversalExpression" /> and returns the result.
    /// </summary>
    public virtual GaussDBJsonTraversalExpression Append(SqlExpression pathComponent)
    {
        ...
        return new GaussDBJsonTraversalExpression(Expression, newPath, ReturnsText, Type, TypeMapping);
    }

    /// <inheritdoc />
    public override bool Equals(object? obj)
        => Equals(obj as GaussDBJsonTraversalExpression);

    /// <inheritdoc />
    public virtual bool Equals(GaussDBJsonTraversalExpression? other)
