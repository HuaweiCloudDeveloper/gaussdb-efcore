...
public class GaussDBJsonDomTranslator : IMemberTranslator, IMethodCallTranslator
{
        ...
        IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
        ...
        {
            return instance is GaussDBJsonTraversalExpression prevPathTraversal
                ? prevPathTraversal.Append(_sqlExpressionFactory.ApplyDefaultTypeMapping(arguments[0]))
                : null;
        }

        if (GetMethods.Contains(method.Name) && arguments.Count == 0 && instance is GaussDBJsonTraversalExpression traversal)
        {
            var traversalToText = new GaussDBJsonTraversalExpression(
