...
public class GaussDBMiscAggregateMethodTranslator : IAggregateMethodCallTranslator
{
        ...
        IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
        ...
        if (method.DeclaringType == typeof(GaussDBAggregateDbFunctionsExtensions))
        {
            switch (method.Name)
            {
                ...
                case nameof(GaussDBAggregateDbFunctionsExtensions.JsonObjectAgg):
                    ...
                    // here.
                    if (source.Selector is not GaussDBRowValueExpression rowValueExpression)
                    {
                        return null;
                    }
        ...
        if (method.DeclaringType == typeof(GaussDBCidrDbFunctionsExtensions))
        {
            switch (method.Name)
            {
                case nameof(GaussDBCidrDbFunctionsExtensions.RangeAgg):
                ...
                case nameof(GaussDBCidrDbFunctionsExtensions.RangeIntersectAgg):
                    return _sqlExpressionFactory.AggregateFunction(
                        "range_intersect_agg",
                        [sqlExpression],
                        source,
