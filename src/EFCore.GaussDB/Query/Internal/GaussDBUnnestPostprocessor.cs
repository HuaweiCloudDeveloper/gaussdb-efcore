...
/// <summary>
///     Locates instances of <see cref="GaussDBUnnestExpression" /> in the tree and prunes the WITH ORDINALITY clause from them if the
        ...
        switch (expression)
        {
            ...
            case SelectExpression selectExpression:
            {
                ...
                for (var i = 0; i < selectExpression.Tables.Count; i++)
                {
                    ...
                    // ordered results.
                    if (unwrappedTable is GaussDBUnnestExpression unnest
                            ...
                            .Any(IsOrdinalityColumn))
                    {
                        ...
                        var newUnnest = new GaussDBUnnestExpression(unnest.Alias, unnest.Array, unnest.ColumnName, withOrdinality: false);

                        newTables[i] = table switch
                        {
                            JoinExpressionBase j => j.Update(newUnnest),
                            GaussDBUnnestExpression => newUnnest,
