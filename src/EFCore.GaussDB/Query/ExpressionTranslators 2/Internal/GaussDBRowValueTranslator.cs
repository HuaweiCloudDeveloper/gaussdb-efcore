...
public class GaussDBRowValueTranslator : IMethodCallTranslator
{
    ...
    /// </summary>
    public GaussDBRowValueTranslator(GaussDBExpressionFactory sqlExpressionFactory)
    {
        ...
        IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
        ...
        {
            return new GaussDBRowValueExpression(arguments, method.ReturnType);
        }
        ...
        var leftCount = arguments[1] is GaussDBRowValueExpression leftRowValue
            ? leftRowValue.Values.Count
            : arguments[1] is SqlConstantExpression { Value : ITuple leftTuple }
                ? (int?)leftTuple.Length
                : null;

        var rightCount = arguments[2] is GaussDBRowValueExpression rightRowValue
