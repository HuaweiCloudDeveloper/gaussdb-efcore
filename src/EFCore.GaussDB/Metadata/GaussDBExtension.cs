...
public class GaussDBExtension
{
    ...
    /// </exception>
    internal GaussDBExtension(IReadOnlyAnnotatable annotatable, string annotationName)
    {
    ...
    /// <summary>
    ///     Gets or adds a <see cref="GaussDBExtension" /> from or to the <see cref="IMutableAnnotatable" />.
    /// </summary>
    ...
    /// <returns>
    ///     The <see cref="GaussDBExtension" /> from the <see cref="IMutableAnnotatable" />.
    /// </returns>
        ...
        string? version)
    {
        ...
        return new GaussDBExtension(annotatable, annotationName) { Version = version };
    }
    ...
    /// <summary>
    ///     Finds a <see cref="GaussDBExtension" /> in the <see cref="IAnnotatable" />, or returns null if not found.
    /// </summary>
    ...
    /// <param name="name">The extension name.</param>
    /// <returns>
    ///     The <see cref="GaussDBExtension" /> from the <see cref="IAnnotatable" />.
    /// </returns>
    /// <exception cref="ArgumentException">
    ///     <paramref name="schema" />
    /// </exception>
    ...
    ///     <paramref name="name" />
    /// </exception>
    public static GaussDBExtension? FindPostgresExtension(
        ...
        string name)
    {
        ...
        return annotatable[annotationName] is null ? null : new GaussDBExtension(annotatable, annotationName);
    }
    ...
    /// <summary>
    ///     Gets the collection of <see cref="GaussDBExtension" /> stored in the <see cref="IAnnotatable" />.
    /// </summary>
    /// <param name="annotatable">The annotatable to search for <see cref="GaussDBExtension" /> annotations.</param>
    /// <returns>
    ///     The collection of <see cref="GaussDBExtension" /> stored in the <see cref="IAnnotatable" />.
    /// </returns>
    /// <exception cref="ArgumentNullException">
    ///     <paramref name="annotatable" />
    /// </exception>
    public static IEnumerable<GaussDBExtension> GetPostgresExtensions(IReadOnlyAnnotatable annotatable)
        => Check.NotNull(annotatable, nameof(annotatable))
            .GetAnnotations()
            .Where(a => a.Name.StartsWith(GaussDBAnnotationNames.PostgresExtensionPrefix, StringComparison.Ordinal))
            .Select(a => new GaussDBExtension(annotatable, a.Name));
