...
/// </summary>
public class GaussDBCollation
{
    private readonly IReadOnlyAnnotatable _annotatable;
    private readonly string _annotationName;

    internal GaussDBCollation(IReadOnlyAnnotatable annotatable, string annotationName)
    {
        ...
        bool? deterministic = null)
    {
        ...
        return new GaussDBCollation(annotatable, annotationName)
        {
            LcCollate = lcCollate,
            LcCtype = lcCtype,
            Provider = provider,
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public static GaussDBCollation? FindCollation(
        ...
        string name)
    {
        ...
        return annotatable[annotationName] is null ? null : new GaussDBCollation(annotatable, annotationName);
    }
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public static IEnumerable<GaussDBCollation> GetCollations(IReadOnlyAnnotatable annotatable)
        => Check.NotNull(annotatable, nameof(annotatable))
            .GetAnnotations()
            .Where(a => a.Name.StartsWith(GaussDBAnnotationNames.CollationDefinitionPrefix, StringComparison.Ordinal))
            .Select(a => new GaussDBCollation(annotatable, a.Name));
