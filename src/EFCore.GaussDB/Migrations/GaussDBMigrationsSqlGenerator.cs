...
#pragma warning restore CS0618
    ...
    public virtual void Generate(GaussDBDropDatabaseOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        ...
        var dbName = operation.Name;
        // System database protection: Block deletion of GaussDB core system databases
        var systemDatabases = new[] { "postgres", "template0", "template1" };
        if (systemDatabases.Contains(dbName.ToLower(CultureInfo.CurrentCulture)))
        {
            throw new InvalidOperationException($"Cannot delete GaussDB system database '{dbName}'. Please use a custom database name.");
        }

        // Properly escape database name to avoid SQL injection and special character issues
        var delimitedDbName = DelimitIdentifier(dbName);
        var escapedDbName = dbName.Replace("'", "''");

        // Generate GaussDB-compatible SQL (complete the condition to exclude current session)
        builder.AppendLine($"REVOKE CONNECT ON DATABASE {delimitedDbName} FROM PUBLIC;");
        // Critical fix: Add pid <> pg_backend_pid() to avoid terminating the current session
        builder.AppendLine($"SELECT pg_terminate_backend(pid) FROM pg_stat_activity " +
                           $"WHERE datname = '{escapedDbName}' AND pid <> pg_backend_pid();");
        builder.EndCommand(suppressTransaction: true);
        // Add IF EXISTS to avoid errors when the database does not exist
        builder.AppendLine($"DROP DATABASE IF EXISTS {delimitedDbName};");
    ...
    /// <inheritdoc />
    protected virtual void GenerateCreateCollation(GaussDBCollation collation, IModel? model, MigrationCommandListBuilder builder)
    {
    ...
    /// <inheritdoc />
    protected virtual void GenerateDropCollation(GaussDBCollation collation, IModel? model, MigrationCommandListBuilder builder)
    {
        var schema = collation.Schema ?? model?.GetDefaultSchema();
    ...
    /// <inheritdoc />
    protected virtual void GenerateCreateEnum(GaussDBEnum enumType, IModel? model, MigrationCommandListBuilder builder)
    {
    ...
    /// <inheritdoc />
    protected virtual void GenerateDropEnum(GaussDBEnum enumType, IModel? model, MigrationCommandListBuilder builder)
    {
        var schema = enumType.Schema ?? model?.GetDefaultSchema();
    ...
    /// <inheritdoc />
    protected virtual void GenerateCreateRange(Metadata.GaussDBRange rangeType, IModel? model, MigrationCommandListBuilder builder)
    {
    ...
    /// <inheritdoc />
    protected virtual void GenerateDropRange(Metadata.GaussDBRange rangeType, IModel? model, MigrationCommandListBuilder builder)
    {
        var schema = rangeType.Schema ?? model?.GetDefaultSchema();
        ...
        MigrationCommandListBuilder builder)
    {
        ...
        if (valueGenerationStrategy.IsIdentity() || valueGenerationStrategy == GaussDBValueGenerationStrategy.SerialColumn)
        {
            ...
            valueGenerationStrategy = GaussDBValueGenerationStrategy.SerialColumn;
        }
...
#pragma warning restore CS0618
        ...
        {
            // Serial 列强制非空（无需调用 IdentityDefinition）
        ...
        MigrationCommandListBuilder builder)
    {
        ...
        if (operation[GaussDBAnnotationNames.IdentityOptions] is string identitySequenceOptions)
        {
            ...
            // Note: in older versions of GaussDB there's a slight variation, see GaussDBDatabaseModelFactory.
...
#pragma warning disable 618
    ...
    private static void CheckForOldValueGenerationAnnotation(IAnnotatable annotatable)
    {
        ...
        {
            throw new NotSupportedException(
                "The GaussDB:ValueGeneratedOnAdd annotation has been found in your migrations, but is no longer supported. Please replace it with '.Annotation(\"GaussDB:ValueGenerationStrategy\", GaussDBValueGenerationStrategy.SerialColumn)' where you want GaussDB serial (autoincrement) columns, and remove it in all other cases.");
        }
    }
