...
Install the provider with the `HuaweiCloud.GaussDB.EntityFrameworkCore` package. The namespace and API remain `HuaweiCloud.EntityFrameworkCore.GaussDB`, so existing `UseGaussDB(...)` code does not need to change when migrating from the previous `HuaweiCloud.EntityFrameworkCore.GaussDB` package ID.
