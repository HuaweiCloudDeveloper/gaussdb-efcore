...
namespace HuaweiCloud.EntityFrameworkCore.GaussDB.Extensions.Internal;
...
public static class GaussDBShapedQueryExpressionExtensions
{
        ...
        bool ignorePredicate = false)
    {
        if (source.QueryExpression is SelectExpression
            {
                Tables: [GaussDBUnnestExpression { Array: var a } unnest],
                GroupBy: [],
                Having: null,
                IsDistinct: false,
                Limit: null,
        ...
        bool ignorePredicate = false)
    {
            ...
            && (ignoreOrderings || select.Orderings is []))
        {
            ...
            array = new GaussDBNewArrayExpression(elements, valuesExpression.RowValues[0].Values[1].Type.MakeArrayType(), typeMapping: null);
            return true;
        }
