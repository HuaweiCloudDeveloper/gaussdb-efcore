        ...
        CSharpRuntimeAnnotationCodeGeneratorParameters parameters)
    {
        ...
        switch (typeMapping)
        {
            ...
            case GaussDBRangeTypeMapping rangeTypeMapping:
            {
                ...
                var npgsqlDbTypeDifferent = rangeTypeMapping.GaussDBDbType != defaultInstance.GaussDBDbType;
        ...
        void CheckElementTypeMapping()
        {
            if (_typeMappingNestingCount > 1)
            {
                throw new NotSupportedException(
                    $"Non-default GaussDB type mappings ('{typeMapping.GetType().Name}' with store type '{(typeMapping as RelationalTypeMapping)?.StoreType}') aren't currently supported as element type mappings, see https://github.com/npgsql/efcore.pg/issues/3366.");
            }
        }
    }
