...
#pragma warning restore EF1001
        ...
        ParameterDirection direction = ParameterDirection.Input)
    {
        ...
        if (param is not GaussDBParameter npgsqlParameter)
        {
            throw new InvalidOperationException(
                $"GaussDB-specific type mapping {GetType().Name} being used with non-GaussDB parameter type {param.GetType().Name}");
        }

        // Enums and user-defined ranges require setting GaussDBParameter.DataTypeName to specify the GaussDB type name.
    ...
    protected override void ConfigureParameter(DbParameter parameter)
    {
        ...
        {
            npgsqlParameter.GaussDBDbType = GaussDBDbType.Value;
        }
    }
}
