...
public class GaussDBDataSourceManager : IDisposable, IAsyncDisposable
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual DbDataSource? GetDataSource(GaussDBOptionsExtension? npgsqlOptionsExtension, IServiceProvider? applicationServiceProvider)
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    protected virtual GaussDBDataSource CreateDataSource(GaussDBOptionsExtension npgsqlOptionsExtension)
    {
        var dataSourceBuilder = new GaussDBDataSourceBuilder(npgsqlOptionsExtension.ConnectionString);
