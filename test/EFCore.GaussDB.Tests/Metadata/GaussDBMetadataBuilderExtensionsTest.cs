...
public class GaussDBInternalMetadataBuilderExtensionsTest
{
    ...
    public void Can_access_model()
    {
        ...
        Assert.NotNull(
            builder
                .HasValueGenerationStrategy(GaussDBValueGenerationStrategy.SequenceHiLo));
        Assert.Equal(GaussDBValueGenerationStrategy.SequenceHiLo, builder.Metadata.GetValueGenerationStrategy());

        Assert.NotNull(
            builder
                .HasValueGenerationStrategy(GaussDBValueGenerationStrategy.IdentityByDefaultColumn, fromDataAnnotation: true));
        Assert.Equal(
            GaussDBValueGenerationStrategy.IdentityByDefaultColumn, builder.Metadata.GetValueGenerationStrategy());

        Assert.Null(
            builder
                .HasValueGenerationStrategy(GaussDBValueGenerationStrategy.SequenceHiLo));
        Assert.Equal(GaussDBValueGenerationStrategy.IdentityByDefaultColumn, builder.Metadata.GetValueGenerationStrategy());

        Assert.Equal(
            1, builder.Metadata.GetAnnotations().Count(
                a => a.Name.StartsWith(GaussDBAnnotationNames.Prefix, StringComparison.Ordinal)));
    }
